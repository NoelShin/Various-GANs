...
class Generator(nn.Module):
    ...
    def set_weight(self):
        ...
        self.dconv_3 = nn.ConvTranspose2d(2 * 8 * n_gf, 4 * n_gf, kernel_size=4, padding=1, stride=2, bias=False)  # 1x256x32x32
        self.dconv_2 = nn.ConvTranspose2d(2 * 4 * n_gf, 2 * n_gf, kernel_size=4, padding=1, stride=2, bias=False)  # 1x128x64x64
        self.dconv_1 = nn.ConvTranspose2d(2 * 2 * n_gf, n_gf, kernel_size=4, padding=1, stride=2, bias=False)  # 1x64x128x128
        self.last_dconv = nn.ConvTranspose2d(n_gf, output_channel, kernel_size=4, padding=1, stride=2, bias=False)  # 1x3x128x128

    def build_model(self):
        self.first_block = block(self.conv_1, relu=False)    # 1x128x64x64
        self.second_block = block(self.conv_2, relu=False)  # 1x256x32x32
        self.third_block = block(self.conv_3, relu=False)  # 1x512x64x64
        ...
        self.seventh_dblock = block(self.dconv_7, transpose=True, dropout=True)
        self.sixth_dblock = block(self.dconv_6, transpose=True, dropout=True)
        self.fifth_dblock = block(self.dconv_5, transpose=True, dropout=True)
        self.fourth_dblock = block(self.dconv_4, transpose=True)
        self.third_dblock = block(self.dconv_3, transpose=True)
        self.second_dblock = block(self.dconv_2, transpose=True)
        self.first_dblock = block(self.dconv_1, transpose=True)

    def forward(self, x):
        d0 = self.first_conv(x)  # 1x64x128x128
        d1 = self.first_block(d0)  # 1x128x64x64
        d2 = self.second_block(d1)  # 1x256x32x32
        d3 = self.third_block(d2)  # 1x512x16x16
        d4 = self.fourth_block(d3)  # 1x512x8x8
        d5 = self.fifth_block(d4)  # 1x512x4x4
        d6 = self.sixth_block(d5)  # 1z512x2x2
        d7 = self.seventh_block(d6)  # 1x512x1x1
        u7 = self.seventh_dblock(d7)  # 1x512x2x2
        u6 = self.sixth_dblock(torch.cat([u7, d6], dim=1))  # 1x512x4x4
        u5 = self.fifth_dblock(torch.cat([u6, d5], dim=1))  # 1x512x8x8
        u4 = self.fourth_dblock(torch.cat([u5, d4], dim=1))  # 1x512x16x16
